...
import { getTicketImageOptions } from '../../utils/ticketImageUtils';
...
const Ticket = (props, this_ref) => {
  ...
  useImperativeHandle(this_ref, () => ({
    renderTicket: () => {
      ...
      return toPng(htmlRef.current, getTicketImageOptions(500, 250));
    },
  }));
  ...
  useEffect(() => {
    if (onRender) {
      ...
      toPng(ref.current, getTicketImageOptions(500, 250))
        .then(dataUrl => {
          ref.current.style.transform = 'none';
          onRender(dataUrl);
        })
